...
export default function AIAssistantUI() {
  ...
  function createNewChat() {
    ...
    const item = {
      id,
      title: "New agenda drop",
      updatedAt: new Date().toISOString(),
      messageCount: 0,
      preview: "Post the stat. We'll do the rest.",
      pinned: false,
  ...
  async function sendMessage(convId, content) {
    ...
    const outgoingMessages = [...(currentConv?.messages || []), userMsg]
    const payloadMessages = outgoingMessages.map((m) => ({ role: m.role, content: m.content }))
    ...
    try {
      ...
      const reply = typeof data?.content === "string" && data.content.trim() ? data.content.trim() : "No response."
      setConversations((prev) =>
        prev.map((c) => {
      ...
      const fallback = "Groq had a moment. Drop the stat again."
      setConversations((prev) =>
        prev.map((c) => {
          if (c.id !== currentConvId) return c
          const asstMsg = {
    ...
    <div className="h-screen w-full bg-zinc-50 text-zinc-900 dark:bg-zinc-950 dark:text-zinc-100">
      <div className="md:hidden sticky top-0 z-40 flex items-center gap-2 border-b border-zinc-200/60 bg-white/80 px-3 py-2 backdrop-blur dark:border-zinc-800 dark:bg-zinc-900/70">
        <div className="ml-1 flex items-center gap-2 text-sm font-semibold tracking-tight">
          <span className="inline-flex h-4 w-4 items-center justify-center">✱</span> Chokli AI (Certified Stat Bully)
        </div>
      ...
      <div className="flex h-[calc(100vh-0px)] w-full max-w-none">
        <Sidebar
          open={sidebarOpen}
