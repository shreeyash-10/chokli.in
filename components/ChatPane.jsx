...
const ChatPane = forwardRef(function ChatPane(
  ...
  const tags = ["Receipts Verified", "Tailored to Your Bias", "Seen This Argument Since 2013", "Hurts Feelings, Saves Facts"]
  const messages = Array.isArray(conversation.messages) ? conversation.messages : []
  const displayTitle = conversation.title === "New agenda drop" ? "Drop a stat, coward" : conversation.title
    ...
    <div className="flex h-full min-h-0 flex-1 flex-col">
      <div className="flex-1 space-y-5 overflow-y-auto px-4 py-6 sm:px-8">
        <div className="mb-2 text-3xl font-serif tracking-tight sm:text-4xl md:text-5xl">
          <span className="block leading-[1.05] font-sans text-2xl">{displayTitle}</span>
        </div>
        <div className="mb-4 text-sm text-zinc-500 dark:text-zinc-400">
          Updated just now · lies pending
        </div>
        ...
        {messages.length === 0 ? (
          <div className="rounded-xl border border-dashed border-zinc-300 p-6 text-sm text-zinc-500 dark:border-zinc-700 dark:text-zinc-400">
            Post the stat. We'll do the rest.
          </div>
        ) : (
          <>
            {messages.map((m) => (
